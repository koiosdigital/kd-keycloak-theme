import { render } from "jsx-email";
import { createVariablesHelper } from "keycloakify-emails/variables";
import type { GetSubject, GetTemplate } from "keycloakify-emails";
import { EmailLayout, EmailHeading, EmailText, colors } from "../components";
import { Section, Text } from "jsx-email";

const { exp } = createVariablesHelper("event-user_disabled_by_temporary_lockout.ftl");

function EventUserDisabledTemporaryEmail() {
    return (
        <EmailLayout preview="Account temporarily locked" realmName={exp("realmName")}>
            <EmailHeading>Account Temporarily Locked</EmailHeading>
            <EmailText>
                Hello {exp("user.firstName")},
            </EmailText>
            <EmailText>
                Your <strong>{exp("realmName")}</strong> account has been temporarily
                locked due to too many failed login attempts.
            </EmailText>
            <Section style={warningBoxStyle}>
                <Text style={warningTextStyle}>
                    <strong>Time:</strong> {exp("event.date")}
                </Text>
                <Text style={warningTextStyle}>
                    <strong>IP Address:</strong> {exp("event.ipAddress")}
                </Text>
            </Section>
            <EmailText>
                Please wait a few minutes before attempting to log in again. If you
                continue to have trouble, contact your administrator.
            </EmailText>
        </EmailLayout>
    );
}

const warningBoxStyle: React.CSSProperties = {
    backgroundColor: "rgba(245, 158, 11, 0.1)",
    border: "1px solid rgba(245, 158, 11, 0.25)",
    borderRadius: "8px",
    padding: "16px",
    margin: "16px 0"
};

const warningTextStyle: React.CSSProperties = {
    color: colors.neutral700,
    fontSize: "14px",
    lineHeight: "22px",
    margin: "4px 0"
};

export const getTemplate: GetTemplate = async props => {
    if (props.plainText) {
        return `Account Temporarily Locked

Hello ${exp("user.firstName")},

Your ${exp("realmName")} account has been temporarily locked due to too many failed login attempts.

Time: ${exp("event.date")}
IP Address: ${exp("event.ipAddress")}

Please wait a few minutes before attempting to log in again.`;
    }
    return render(<EventUserDisabledTemporaryEmail />);
};

export const getSubject: GetSubject = async () => {
    return `Your ${exp("realmName")} account is temporarily locked`;
};
